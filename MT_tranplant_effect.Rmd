...
These genes will be those that are looked at in the heatmap and upset plot.
...
There are lots of genes which are only on one, or a subset of comparisons, but there are 77 (in red) which are common to all swaps. 
...
Now to look at the 71 genes that were identified as being enriched in every swap compared to the baseline conventional treatment (above, in red). We don't know if they are enriched in the same direction at all, or whether the swaps show both up and down regulation of these genes compared to conventional
...
```{r heatmap_71_genes_to_conv_scale_none}


# create heatmap
heatmap_77_genes_to_conv_scale_none <- pheatmap(
...
heatmap_77_genes_to_conv_scale_none

```


All 77 geneIDs identified as deferentially expressed in all comparisons to conventional, pretty much have consistent directions of change.
...
col_fun = colorRamp2(c(-15, -1.5, -1.499, 1.499, 1.5, 15), c( "#283593", "#C5CAE9","snow2", "snow2", "#FFE0B2", "#E65100"))
col_fun(seq(-15, 15))


heatmap_1680_genes_to_conv_rownames <- Heatmap(bound.padj.no.nas, name = "mat", col = col_fun, row_names_gp = gpar(fontsize = 3))

heatmap_1680_genes_to_conv <- Heatmap(bound.padj.no.nas, name = "log2fold change", col = col_fun, show_row_names=F)
...
col_fun2 = colorRamp2(c(-10, -1.5, -1.499, 1.499, 1.5, 15), c( "#283593", "#C5CAE9","snow2", "snow2", "#FFE0B2", "#E65100"))
col_fun2(seq(-10, 15))


heatmap_shared_to_conv_rownames <- Heatmap(bound.shared.to.conv, name = "log2foldchange", col = col_fun, row_names_gp = gpar(fontsize = 3))

heatmap_shared_to_conv_rownames

```

...and with no rownames
...
heatmap_shared_to_conv_no_rownames <- Heatmap(bound.shared.to.conv, name = "log2foldchange", col = col_fun, show_row_names=F)
